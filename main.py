...
async def translate_directory(
    ...
    doc_translator = DocumentTranslator(
        ...
        max_concurrent=max_concurrent,  # 传递最大并发参数
    )
    # 打印所有参数
    print(
        f"参数列表:  source_lang={source_lang}, target_lang={target_lang}, incremental={incremental}, auto_commit={auto_commit}, auto_push={auto_push}, commit_message={commit_message}, directory_path={directory_path}, ignore_patterns={ignore_patterns}, output_directory={output_directory}, max_concurrent={max_concurrent}"
    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="AITdocs - 翻译文本、文档或目录中的Markdown文件"
    )
    ...
    # 公共参数
    parser.add_argument(
        "-s", "--source-lang", default="auto", help="源语言代码（默认：auto）"
    )
    parser.add_argument(
        "-l", "--target-lang", default="zh", help="目标语言代码（默认：zh）"
    )
    parser.add_argument(
        "-o", "--output", help="输出文件路径（文本/文件模式）或输出目录路径（目录模式）"
    )
    parser.add_argument("-i", "--ignore", nargs="*", help="目录模式下的忽略规则列表")
    parser.add_argument(
        "--incremental",
        action="store_true",
        help="启用增量翻译模式（仅翻译变更的文件）",
    )
    parser.add_argument(
        "--auto-commit", action="store_true", help="翻译完成后自动提交到Git仓库"
    )
    parser.add_argument(
        "--commit-message",
        default="Update translated documents",
        help="自动提交的提交信息",
    )
    parser.add_argument(
        "--auto-push",
        action="store_true",
        help="自动推送到远程仓库（需要先启用--auto-commit）",
    )
    parser.add_argument(
        "--max-concurrent", type=int, default=5, help="最大并发翻译任务数（默认：5）"
    )
